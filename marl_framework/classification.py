...
class Classification:
    ...
    def split_dataset(self, batch_memory):
        ...
        for c_i, i in enumerate(training_set_indices):
            print(f"c_i: {c_i}")
            training_input, training_labels = add_data(
                self.params,
                training_input,
                training_labels,
                batch_memory,
        ...
        validation_set_indices = sample_indices[
            ...
            + int(batch_memory.size() * self.data_split[1])
        ]
        ...
        for c_i, i in enumerate(validation_set_indices):
            print(f"j: {c_i}")
            validation_input, validation_labels = add_data(
                self.params,
                validation_input,
                validation_labels,
                batch_memory,
        ...
        testing_set_indices = sample_indices[
            ...
            + int(batch_memory.size() * self.data_split[2])
        ]
        testing_input = []
        testing_labels = []
        ...
        for c_i, i in enumerate(testing_set_indices):
            print(f"k: {c_i}")
            testing_input, testing_labels = add_data(
                self.params,
                testing_input,
                testing_labels,
                batch_memory,
            ...
            for tp in training_phases:
                ...
                for batch_input, batch_label in zip(batches_inputs, batches_labels):
                    ...
                    if self.input_type == "mixed":
                        batch_input1 = batch_input[
                            :, :, : self.agent_state_space.space_x_dim, :
                        ]
                        batch_input2 = batch_input[
                            :, :, self.agent_state_space.space_x_dim :, :
                        ]
                        classification = self.model.forward(
                            batch_input1.float(),
                            batch_input2[:, 0, :, 0].float(),
                            self.path.split("/")[-2],
                        )
                    else:
                        classification = self.model.forward(
                            batch_input.float(), self.path.split("/")[-2]
                        )
                ...
                precision, recall, f1_score, accuracy, distribution, classified, report, mse = get_metrics(
                    classifications,
                    labels,
                    self.path.split("/")[-2].split("_")[-1],
                    self.path.split("/")[-2].split("_")[0],
                    self.n_actions,
                )

                if (
                    tp == "Validation" and sum(losses) / len(losses) < best_performance
                ):  # f1_score >= best_performance
                    best_epoch = epoch
                ...
                if epoch % 20 == 0:
                    visualize_regression_performance(
                        classifications, labels, tp, epoch, self.params
                    )
    ...
    def evaluate(self):
        ...
        visualize_regression_performance(
            classifications, labels, "Testing", None, self.params
        )
...
def add_data(params, inputs, labels, batch_memory, i, mapping):
    ...
    mlp1 = get_input_mlp1(
        global_input, n_agents, n_actions, agent_state_space, agent_action_space
    )
    ...
    ipp_regression = get_label_ipp_regression(
        global_input, mlp1, agent_state_space, mapping
    )
    ...
    for coordinate in range(len(position_input) // 2):  #
        positions_3d.append(
            agent_state_space.index_to_position(
                [positions[coordinate * 2], positions[coordinate * 2 + 1]]
            )
        )  # , positions[coordinate * 3 + 2]]))

    new_positions = np.zeros_like(positions)
    for pos_index in range(len(positions_3d)):
        action = n_actions * action_input[pos_index] - 1
        new_position = agent_action_space.action_to_position(
            np.squeeze(positions_3d[pos_index]), action
        )
        new_positions[pos_index * 2] = agent_state_space.position_to_index(
            new_position
        )[
            0
        ] / (  #
        ...
        new_positions[pos_index * 2 + 1] = agent_state_space.position_to_index(
            new_position
        )[
            1
        ] / (  #
...
def get_input_cnn1(new_position_input, agent_state_space, n_agents, w_entropy_map):
    ...
    position_map = np.zeros(
        (
            agent_state_space.space_x_dim,
            agent_state_space.space_y_dim,
    ...
    for position in range(len(positions) // 3):
        position_map[
            ...
            int(positions[position * 3 + 1]),
            int(positions[position * 3 + 2]),
        ] = 1
...
def get_input_cnn4(mlp_input, n_agents, agent_state_space, w_entropy_map):
    ...
    for position in range(len(positions) // 2):  #
        position_map[
            int(positions[position * 2]), int(positions[position * 2 + 1])
        ] = 1  # positions[position * 3 + 2] / agent_state_space.space_z_dim
        # if position == 0:  ##### CHANGED FOR ADVANTAGE CALCULATION!!!
        #     pass
        # else:
        action_map[
...
def get_input_mixed(position_map, vector_input, n_agents, agent_state_space):
    ...
    actions = vector_input[n_agents * 2 :].tolist() * np.ones(
        (1, agent_state_space.space_y_dim)
    )
...
def get_label_collision_classification(new_position_input):
    ...
    for coordinate in range(len(new_position_input) // 3):
        positions_3d.append(
            [
                new_position_input[coordinate * 3],
                new_position_input[coordinate * 3 + 1],
...
def get_label_collision_advantages(
    mlp2, n_agents, n_actions, agent_state_space, agent_action_space, params
):
    new_other_positions = []
    rewards = []
    for agent in range(1, n_agents):
        ...
        new_other_positions.append(
            agent_action_space.action_to_position(
                other_position, mlp2[n_agents * 2 + agent] * (n_actions - 1)
            )
        )
    own_position = np.array(
        agent_state_space.index_to_position(
            [
                (mlp2[0] * (agent_state_space.space_x_dim - 1)).item(),
                (mlp2[1] * (agent_state_space.space_y_dim - 1)).item(),
            ]
        )
    )
    n_outliers = 0
...
def get_label_ipp_regression(
    global_input, new_position_input, agent_state_space, mapping
):
    ...
    for coordinate in range(len(positions) // 2):  #
        positions_3d.append(
            agent_state_space.index_to_position(
                [positions[coordinate * 2].item(), positions[coordinate * 2 + 1].item()]
            )
        )  # , positions[coordinate * 3 + 2].item()]))

    updated_maps = []
    for pos_3d in range(len(positions_3d)):
        updated_maps.append(
            mapping.update_grid_map(
                np.array(positions_3d[pos_3d]), map_state.copy(), None
            )
        )
    fused_map = mapping.fuse_map(updated_maps[0], updated_maps[1:])
    mapping_reward = get_utility_reward(
        map_state, fused_map, simulated_map, agent_state_space
    )
    return [mapping_reward * 100, t]


def get_label_ipp_classification(
    global_input,
    mapping,
    n_agents,
    n_actions,
    ...
    own_position = agent_state_space.index_to_position(
        [
            int(
                np.round(
                    (agent_state_space.space_x_dim - 1) * position_input[0].item(), 2
                )
            ),
            int(
                np.round(
                    (agent_state_space.space_y_dim - 1) * position_input[1].item(), 2
                )
            ),
            int(
                np.round(
                    (agent_state_space.space_z_dim - 1) * position_input[2].item(), 2
                )
            ),
        ]
    )
            ...
            agent_state_space.index_to_position(
                [
                    int(
                        np.round(
                            (agent_state_space.space_x_dim - 1)
                            * other_position_input[coordinate * 3].item(),
                            2,
                        )
                    ),
                    int(
                        np.round(
                            (agent_state_space.space_y_dim - 1)
                            * other_position_input[coordinate * 3 + 1].item(),
                            2,
                        )
                    ),
                    int(
                        np.round(
                            (agent_state_space.space_z_dim - 1)
                            * other_position_input[coordinate * 3 + 2].item(),
                            2,
                        )
                    ),
                ]
    ...
    for action_index in range(n_actions):
        ...
        if is_in_map(new_position, params):
            for pos_3d in range(len(positions_3d)):
                new_map = mapping.update_grid_map(
                    np.array(positions_3d[pos_3d]), updated_map, None
                )
                updated_map = new_map.copy()
            mapping_rewards.append(
                get_utility_reward(
                    map_state, updated_map, global_input[2], agent_state_space
                )
            )
        else:
    ...
    for tr in range(len(training_input)):
        for task_index, task in enumerate(tasks):
            for input_type_index, input_type in enumerate(input_types):
                np.savetxt(
                    path
                    + f"/data/{task}/{input_type}/{split_folders[0]}/training_input_{tr}.txt",
                    training_input[tr][input_type_index],
                    fmt="%s",
                )
                np.savetxt(
                    path
                    + f"/data/{task}/{input_type}/{split_folders[1]}/training_labels_{tr}.txt",
                    np.asarray(training_labels[tr][task_index]),
                )  # * np.ones(1))
    ...
    for va in range(len(validation_input)):
        for task_index, task in enumerate(tasks):
            for input_type_index, input_type in enumerate(input_types):
                np.savetxt(
                    path
                    + f"/data/{task}/{input_type}/{split_folders[2]}/validation_input_{va}.txt",
                    validation_input[va][input_type_index],
                    fmt="%s",
                )
                np.savetxt(
                    path
                    + f"/data/{task}/{input_type}/{split_folders[3]}/validation_labels_{va}.txt",
                    np.asarray(validation_labels[va][task_index]),
                )  #  * np.ones(1))
    ...
    for te in range(len(testing_input)):
        for task_index, task in enumerate(tasks):
            for input_type_index, input_type in enumerate(input_types):
                np.savetxt(
                    path
                    + f"/data/{task}/{input_type}/{split_folders[4]}/testing_input_{te}.txt",
                    testing_input[te][input_type_index],
                    fmt="%s",
                )
                np.savetxt(
                    path
                    + f"/data/{task}/{input_type}/{split_folders[5]}/testing_labels_{te}.txt",
                    np.asarray(testing_labels[te][task_index]),
                )  #  * np.ones(1))
...
def read_training_data(path, experiment_run):
    ...
    for training_input, training_label in zip(
        ...
        training_inputs.append(
            np.genfromtxt(path + experiment_run + f"/training_input/{training_input}")
        )
        try:
...
def read_testing_data(path, experiment_run):
    ...
    for testing_input, testing_label in zip(
        ...
        testing_inputs.append(
            np.genfromtxt(path + experiment_run + f"/testing_input/{testing_input}")
        )
        try:
...
def visualize_regression_performance(classifications, labels, phase, epoch, params):
    ...
    flat_classifications = [
        classification for batch in classifications for classification in batch
    ]

    plt.figure()
    if phase == "Training":
        plt.scatter(
            flat_labels, flat_classifications, color="g"
        )  # , linestyle="-", linewidth=2, label="training")
    elif phase == "Validation":
        plt.scatter(
            flat_labels, flat_classifications, color="r"
        )  # , linestyle="-", linewidth=2, label="validation")
    else:
        plt.scatter(
            flat_labels, flat_classifications, color="orange"
        )  # , linestyle="-", linewidth=2, label="testing")

    plt.plot(
        flat_labels, flat_labels, color="b", linestyle="--", linewidth=1, label="labels"
    )
