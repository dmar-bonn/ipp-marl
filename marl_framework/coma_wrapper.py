...
from marl_framework.q_function.learner import QLearner
from marl_framework.q_function.network import QNetwork
from marl_framework.v_function.learner import VLearner
from marl_framework.v_function.network import VNetwork

from actor.transformations import get_network_input as get_actor_input
from q_function.transformations import get_network_input as get_q_input
from v_function.transformations import get_network_input as get_v_input
...
class COMAWrapper:
    def __init__(self, params: Dict, writer: SummaryWriter):
        ...
        if self.mission_type is "CentralQV":
            self.v_network = VNetwork(self.params)
            self.target_v_network = copy.deepcopy(self.v_network)
            self.v_learner = VLearner(self.params, writer, self.v_network)
