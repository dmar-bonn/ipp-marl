...
class Mapping:
    ...
    def update_grid_map(self, position, map_state, t, mode):
        ...
        footprint_img = np.ones((footprint[1] - footprint[0], footprint[3] - footprint[2])) * 0.5

        # Extract map section within camera footprint
        map_section = map_state[
                      footprint_clipped[2]: footprint_clipped[3], footprint_clipped[0]: footprint_clipped[1]
                      ]
        measurement = self.simulation.get_measurement(position[2], footprint_clipped, mode)

        # Update cells of extracted map section
        cell_update = self.update_cells(
            map_section, measurement, mode
        )
        ...
        map2communicate[footprint_clipped[2]: footprint_clipped[3], footprint_clipped[0]: footprint_clipped[1]] = measurement

        footprint_fixed = get_fixed_footprint_coordinates(footprint, footprint_clipped)
        footprint_img[footprint_fixed[2]: footprint_fixed[3], footprint_fixed[0]: footprint_fixed[1]] = measurement
        ...
        if fusion_mode == "local":  # local map fusion
            ...
            if isinstance(other_map_states, dict):
                ...
                for other_state in other_map_states:
                    other_map = np.float32(other_map_states[other_state]["map2communicate"])
    ...
    def apply_update(self, x, y, mode):
        ...
        # if mode == "train":
        # update_trust = np.copy(update)
        # x_trust = np.copy(x)
        # np.putmask(update_trust, (1 - update_trust) < update_trust, 1 - update_trust)
        # np.putmask(x_trust, (1 - x_trust) < x_trust, 1 - x_trust)
        # np.putmask(update, update_trust > x_trust, x)
        return update
